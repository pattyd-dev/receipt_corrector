  ...
  container_definitions = jsonencode([
    {
      ...
      portMappings = [
        {
          containerPort = 5000
          hostPort      = 0        # 0 = dynamic port assignment, EC2/bridge mode only
          protocol      = "tcp"
        }
      ]
...
# ALB Security Group — Configured in networking/security.tf
...
resource "aws_route53_record" "ecs_receipt_corrector" {
  ...
  name    = "ecs-corrector.${var.domain_name}"    # e.g. "app.example.com"
  type    = "A"
